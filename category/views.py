...
class CategoryProductByIdView(APIView):
    ...
    def get(self, request, category_id):
        ...
        if category_id is None:
            return Response(
                {"status": 400, "message": "category 값이 필요합니다."}, status=400
            )

        try:
            category = Category.objects.prefetch_related("product_set").get(
                id=category_id
            )
        except Category.DoesNotExist:
            return Response(
                {"status": 404, "message": "해당 카테고리는 존재하지 않습니다."},
                status=404,
            )
...
class CategoryView(ListCreateAPIView):
    ...
    def post(self, request):
        ...
        serializer.save()
        return Response(
            {"message": "카테고리가 생성되었습니다."}, status=status.HTTP_201_CREATED
        )
    """
